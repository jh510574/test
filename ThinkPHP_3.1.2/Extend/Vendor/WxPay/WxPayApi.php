...
class WxPayApi
{
  ...
  public static function unifiedOrder($inputObj, $timeOut = 6)
  {
    ...
    if(!$inputObj->IsAppidSet()){
      $inputObj->SetAppid(C('WX_PAY_CONFIG.APPID'));
    }
    if(!$inputObj->IsMch_idSet()){
      $inputObj->SetMch_id(C('WX_PAY_CONFIG.MCHID'));
    }
  ...
  public static function orderQuery($inputObj, $timeOut = 6)
  {
    ...
    $inputObj->SetAppid(C('WX_PAY_CONFIG.APPID'));//公众账号ID
  ...
  {
    //获取通知的数据
    if(empty($GLOBALS['HTTP_RAW_POST_DATA'])){return;}
  ...
  {
    //如果不需要上报数据
    if(C('WX_PAY_CONFIG.REPORT_LEVENL') == 0){
      return;
    }
    //如果仅失败上报
    if(C('WX_PAY_CONFIG.REPORT_LEVENL') == 1 &&
  ...
  private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
  {
    ...
    //如果有配置代理这里就设置代理
    if(C('WX_PAY_CONFIG.CURL_PROXY_HOST') != "0.0.0.0"
      && C('WX_PAY_CONFIG.CURL_PROXY_PORT') != 0){
      curl_setopt($ch,CURLOPT_PROXY, C('WX_PAY_CONFIG.CURL_PROXY_HOST'));
      curl_setopt($ch,CURLOPT_PROXYPORT, C('WX_PAY_CONFIG.CURL_PROXY_PORT'));
    }
    ...
    if($useCert == true){
      ...
      curl_setopt($ch,CURLOPT_SSLCERTPASSWD, C('WX_PAY_CONFIG.MCHID'));
      curl_setopt($ch,CURLOPT_SSLCERT, C('WX_PAY_CONFIG.SSLCERT_PATH'));
      curl_setopt($ch,CURLOPT_SSLKEY, C('WX_PAY_CONFIG.SSLKEY_PATH'));
    }
    //post提交方式
    ...
    if($data){
      ...
      $error = curl_error($ch);
      curl_close($ch);
      throw new WxPayException("curl出错, $error");
    }
  }
